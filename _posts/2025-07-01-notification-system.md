...
1.  iOS 푸시 알림  
    ...
    * 알림 제공자(Provider)
        *  말 그대로 알림을 만들어서 보내는 주체를 말한다. 보통 서버(API 서버, 백엔드 서버)가 이 역할을 담당한다. 알림 제공자는 "새로운 메시지가 도착했어요!" 라는 알림 내용을 만들고, 이 알림을 Apple의 푸시 서버(APNS)에 전달한다.
    * APNS(Apple Push Notification)
        ...
        * APNS는 일종의 우체국 역할이라고 생각하면 된다. 알림 제공자가 보낸 "편지(알림)"을 받아서 아이폰이라는 "수신자"에게 정확하게 전달한다.
    * iOS 단말
...
- 이메일 전송 플랫폼 (SendGrid, Mailchimp 등)
  ...
  _ 이메일이 받는 사람의 스팸함이 아닌, 받은 편지함으로 도착하도록 최적화 작업을 진행한다.
  _ 이메일이 제대로 도착했는지, 이메일을 열었는지, 반송되었는지를 추적할 수 있다.
